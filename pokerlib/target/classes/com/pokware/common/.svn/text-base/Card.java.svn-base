...
public class Card {
	...
	public Card(String s) {
		if (s.length() == 2)
			this.index = chars2index(s.charAt(0), s.charAt(1));
	}
	...
	public Card(char rank, char suit) {
		this.index = chars2index(rank, suit);
	}
	...
	private int chars2index(char rank, char suit) {
		...
		if (s != -1 && r != -1)
			...
			throw new IllegalArgumentException("suit = " + suit + " and rank = " + rank);
	}
