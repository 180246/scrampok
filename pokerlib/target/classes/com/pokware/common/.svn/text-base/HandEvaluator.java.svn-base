...
public class HandEvaluator {
	 ...
	 *
	 * @param hand Any 7 card poker hand
	 * @return A Hand containing the highest ranked 5 card hand possible from the input.
	 */
	public Hand getBest5CardHand(Hand hand) {
		int[] handCardArray = hand.getCardArray();
		int[] bh = new int[6];
		findHand(handCardArray, bh);
		Hand newHand = new Hand();
		for (int i = 0; i < 5; i++) {
			newHand.addCard(bh[i + 1]);
		}
		return newHand;
	}
	...
	private int findHand(int[] hand, int[] best) { /* -1 means unknown card */
